...
/**
 * Public, self-service intro-call booking. A prospect picks an open slot on the
 * founder's real calendar and leaves their details — no demo, no account. The
 * slot list and the booking both hit /api/booking, which is backed by the same
 * founder-calendar engine the admin portal reads, so a booking here shows up in
 * the founder's calendar instantly (and can't double-book).
 */
...
/** "America/Vancouver" → "Vancouver" for a friendly timezone label. */
...
export function BookCallSection() {
  ...
  // Load founder availability once on mount.
  useEffect(() => {
    let alive = true;
    BookingApi.slots(14)
      .then((res) => {
  ...
  async function confirm() {
    ...
    try {
      ...
      // SLOT_TAKEN means someone grabbed it first — refresh availability so the
      // prospect sees the slot disappear and can pick another.
      ...
      if (code === 'SLOT_TAKEN') {
        ...
        setSubmitError(err instanceof ApiError ? err.message : 'Could not book that time. Please try again.');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="mx-auto max-w-4xl overflow-hidden rounded-3xl border border-line/70 bg-white shadow-card">
      <div className="grid md:grid-cols-[1.05fr_0.95fr]">
        {/* ------------------------------ left: calendar ----------------------------- */}
        <div className="border-b border-line/60 p-6 md:border-b-0 md:border-r md:p-8">
          ...
          {loadError ? (
            ...
            <div className="mt-6 space-y-3">
              <div className="h-9 w-full animate-pulse rounded-xl bg-paper" />
              <div className="grid grid-cols-3 gap-2">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="h-9 animate-pulse rounded-lg bg-paper" />
                ))}
              </div>
            </div>
          ) : bookableDays.length === 0 ? (
            <p className="mt-6 text-sm text-ink-muted">
              No open times in the next two weeks — please check back soon, or try the live demo above.
            </p>
          ) : (
            <>
                ...
                {bookableDays.map((d) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex shrink-0 flex-col items-center rounded-xl border px-3.5 py-2 transition-all ${
                        selected
                          ? 'border-signal bg-signal text-white shadow-pop'
                          : 'border-line bg-white text-ink hover:border-signal/40'
                      }`}
                    >
                      <span className={`text-[11px] font-semibold uppercase ${selected ? 'text-white/80' : 'text-ink-muted'}`}>
                        {weekday}
                      </span>
                      <span className="text-[15px] font-bold tracking-tight">{`${mm}/${dd}`}</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div className="p-6 md:p-8">
          {!activeTime ? (
            <div className="flex h-full flex-col items-center justify-center py-10 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-paper ring-1 ring-inset ring-ink/5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="h-6 w-6 text-ink-muted">
                  <rect x="3" y="5" width="18" height="16" rx="3" />
                  <path d="M3 10h18M8 3v4M16 3v4" strokeLinecap="round" />
                </svg>
              </div>
              <p className="mt-4 max-w-[220px] text-sm text-ink-muted">
                Pick a day and time on the left, then tell us a little about your business.
              </p>
            </div>
          ) : (
            <>
              ...
              <button
                ...
                onClick={confirm}
                disabled={submitting}
                className="mt-4 w-full rounded-full bg-gradient-to-b from-signal to-signal-deep px-6 py-3.5 text-[15px] font-semibold text-white shadow-pop transition-all duration-200 ease-smooth hover:-translate-y-0.5 hover:shadow-lift disabled:opacity-60"
              >
                {submitting ? 'Booking…' : 'Confirm booking'}
              </button>
              <p className="mt-2.5 text-center text-[11px] text-ink-muted">
                Free intro call · no account needed · we&apos;ll call the number you give us.
              </p>
            </>
          )}
        </div>
...
/* Small labelled input local to this section (keeps the section self-contained). */
function Field({
  label,
  value,
  onChange,
