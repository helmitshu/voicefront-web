...
/* ── viewport hook — avoids CSS-breakpoint tricks that can fail in production */
function useIsDesktop() {
  ...
  return isDesktop; // null during SSR / before hydration
}
...
export function BookCallSection() {
  ...
  function pickTime(t: string) { setActiveTime(t); setSubmitError(null); setMobileStep('details'); }
  ...
  /* ── calendar panel ───────────────────────────────────────────────────── */
  const calendarPanel = (
    <div className="flex flex-col p-6" style={{ padding: isDesktop ? '2rem' : '1.5rem' }}>
      <p style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--color-signal-deep, #0f6b5e)' }}>
        Pick a time
      </p>
      <p style={{ marginTop: '4px', fontSize: '13px', color: 'var(--color-ink-muted, #6b7280)' }}>
      ...
      {loadError ? (
        ...
        <div className="mt-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-10 animate-pulse rounded-lg bg-paper" />)}
          </div>
        </div>
            ...
            {bookableDays.map((d) => {
              ...
              return (
                <button
                  ...
                  className={`flex shrink-0 flex-col items-center rounded-xl border px-4 py-2.5 transition-all ${selected
                ...
                >
                  <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', opacity: selected ? 0.8 : 0.6 }}>{weekday}</span>
                  <span style={{ marginTop: 2, fontSize: '16px', fontWeight: 700, lineHeight: 1 }}>{mm}/{dd}</span>
                </button>
              );
            })}
          </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        background: selected ? 'var(--color-signal-soft, #f0fdf4)' : '#fff',
  ...
  /* ── form panel ───────────────────────────────────────────────────────── */
  const formPanel = (
    <div className="flex flex-col" style={{ padding: isDesktop ? '2rem' : '1.5rem' }}>
      {!activeTime ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '260px', textAlign: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--color-paper, #f9fafb)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(0,0,0,0.08)' }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ width: 28, height: 28, opacity: 0.4 }}>
              <rect x="3" y="5" width="18" height="16" rx="3" />
              <path d="M3 10h18M8 3v4M16 3v4" strokeLinecap="round" />
            </svg>
          </div>
          <p style={{ marginTop: 16, fontSize: '14px', color: 'var(--color-ink-muted, #6b7280)', maxWidth: 180 }}>
        ...
        <div className="flex flex-col gap-4">
          {/* Mobile back button */}
          {!isDesktop && (
            <button type="button" onClick={() => setMobileStep('pick')} className="flex w-fit items-center gap-1.5 text-[13px] font-medium text-ink-muted hover:text-ink">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-signal-soft/50 px-3.5 py-2.5 text-[13px] font-semibold text-signal-deep ring-1 ring-inset ring-signal/15">
            ...
            <button type="button" onClick={() => { setActiveTime(null); setMobileStep('pick'); }}
  ...
  // isDesktop=null means we haven't hydrated yet — show mobile calendar as default
  if (isDesktop) {
    // Desktop: side-by-side, no CSS class tricks
    return (
      <div className="mx-auto max-w-4xl overflow-hidden rounded-3xl border border-line/70 bg-white shadow-card">
        <div style={{ display: 'flex' }}>
          <div style={{ flex: 1, borderRight: '1px solid rgba(0,0,0,0.07)' }}>
            {calendarPanel}
          </div>
          <div style={{ flex: 1 }}>
            {formPanel}
  ...
  // Mobile (or pre-hydration): step-based single panel
  return (
    <div className="mx-auto max-w-4xl overflow-hidden rounded-3xl border border-line/70 bg-white shadow-card">
      {mobileStep === 'pick' ? calendarPanel : formPanel}
      {/* Step dots */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, borderTop: '1px solid rgba(0,0,0,0.07)', padding: '12px 0' }}>
        <span style={{ height: 6, borderRadius: 3, background: mobileStep === 'pick' ? 'var(--color-signal, #0d9488)' : 'var(--color-line, #e5e7eb)', width: mobileStep === 'pick' ? 24 : 12, transition: 'all 0.3s' }} />
        <span style={{ height: 6, borderRadius: 3, background: mobileStep === 'details' ? 'var(--color-signal, #0d9488)' : 'var(--color-line, #e5e7eb)', width: mobileStep === 'details' ? 24 : 12, transition: 'all 0.3s' }} />
      </div>
    </div>
  );
}
