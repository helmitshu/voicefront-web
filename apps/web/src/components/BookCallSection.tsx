...
function Field({ label, value, onChange, placeholder, type = 'text', autoComplete, optional = false }: {
...
export function BookCallSection() {
  ...
  // ── Reusable panel content (called as functions, not JSX variables) ───────────
          ...
          <>
            <div className="mt-5 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {bookableDays.map((d) => {
                ...
                return (
                  <button key={d.date} type="button" onClick={() => pickDate(d.date)} aria-pressed={selected}
                    className={`flex shrink-0 flex-col items-center rounded-xl border px-4 py-2.5 transition-all ${selected ? 'border-signal bg-signal text-white shadow-pop' : 'border-line bg-white text-ink hover:border-signal/40 hover:bg-signal-soft/20'}`}>
                    <span className={`text-[11px] font-semibold uppercase tracking-wide ${selected ? 'text-white/80' : 'text-ink-muted'}`}>{weekday}</span>
                    <span className="mt-0.5 text-[16px] font-bold leading-none">{mm}/{dd}</span>
                  </button>
                );
              })}
            </div>
              ...
              <div className="mt-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {activeDay.slots.map((t) => {
                    ...
                    return (
                      <button key={t} type="button" onClick={() => pickTime(t)} aria-pressed={selected}
                        className={`rounded-xl border py-3 text-[13px] font-semibold transition-all ${selected ? 'border-signal bg-signal-soft text-signal-deep ring-1 ring-inset ring-signal/40' : 'border-line bg-white text-ink hover:border-signal/40 hover:bg-signal-soft/20'}`}>
                        {to12(t)}
                      </button>
                    );
                  })}
  ...
  function FormContent({ showBack }: { showBack: boolean }) {
    if (!activeTime) {
      return (
        <div className="flex h-full min-h-[260px] flex-col items-center justify-center text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-paper ring-1 ring-inset ring-ink/8">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-7 w-7 text-ink-muted/50">
              <rect x="3" y="5" width="18" height="16" rx="3" />
              <path d="M3 10h18M8 3v4M16 3v4" strokeLinecap="round" />
            </svg>
          </div>
          <p className="mt-4 max-w-[180px] text-sm leading-relaxed text-ink-muted">Select a day and time to continue.</p>
        </div>
      );
    }
    return (
      <div className="flex flex-col gap-4">
        {showBack && (
          <button type="button" onClick={() => setMobileStep('pick')}
  ...
  return (
    <div className="mx-auto max-w-4xl overflow-hidden rounded-3xl border border-line/70 bg-white shadow-card">
      ...
      {/* Desktop layout: always side-by-side — two separate containers avoids CSS cascade issues */}
      <div className="hidden md:flex md:divide-x md:divide-line/60">
        <div className="flex flex-1 flex-col p-8">
          <CalendarContent />
        </div>
        <div className="flex flex-1 flex-col p-8">
          <FormContent showBack={false} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 border-t border-line/50 py-3 md:hidden">
        <span className={`h-1.5 rounded-full transition-all duration-300 ${mobileStep === 'pick' ? 'w-6 bg-signal' : 'w-3 bg-line'}`} />
        <span className={`h-1.5 rounded-full transition-all duration-300 ${mobileStep === 'details' ? 'w-6 bg-signal' : 'w-3 bg-line'}`} />
      </div>
    </div>
  );
}
